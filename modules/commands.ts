...
newCommand("inspect", "begins your inspection timer", async (message) => {
  ...
  for (const s of WARNINGS) {
    setTimeout(() => {
      if (inspecting.get(userId) == startTime) {
        message.channel.send(
          `${username}, you have used ${s} seconds of inspection!`
        );
      }
    }, s * 1000);
  }
...
newCommand(
  "viewsolve",
  "`[user mention] [solve number]` view user's solve",
  async (message) => {
...
newCommand(
  "setmethod",
  "`[method]` sets your solving method in your profile",
  async (message) => {
    const args = parseCommand(message.content);
    ...
    if (await db.setMethod(message.author.id, method)) {
      message.channel.send(
        `${message.author.username}, your solving method has been set to ${method}.`
      );
    } else {
      message.channel.send(
        "Invalid method provided; solving method unchanged."
      );
    }
  }
);
...
newCommand("remove", "removes your last solve", (message) => {
  ...
  if (!solver.solves.empty()) {
    ...
    message.channel.send(
      message.author.username +
        ", your last solve has been removed.\n" +
        "The removed solve is shown below:\n" +
        lastSolve.toString()
    );
  } else {
    message.channel.send(
      `${message.author.username}, you do not have an existing solve.`
    );
  }
});
...
newCommand("+2", "changes whether your last solve was a +2", (message) => {
  ...
  if (db.togglePlusTwo(message.author.id)) {
    ...
    message.channel.send(
      `${message.author.username}, ` +
        `+2 was ${
          se.plusTwo ? "added to" : "removed from"
        } your last solve.\n` +
...
const COMMANDS_STRING = Array.from(COMMANDS.values())
  .map((cmd) => cmd.helpString)
  .join("\n");
...
function canRequest(userId: string): boolean {
  return (
    !lastRequest.has(userId) ||
    Date.now() - lastRequest.get(userId) >= config.COOLDOWN
  );
}
